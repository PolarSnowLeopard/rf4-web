...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['clear']:
            deleted_count = Rod.objects.all().delete()[0]
            self.stdout.write(self.style.SUCCESS(f'已删除 {deleted_count} 条现有渔竿数据'))

        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                rod_data = json.load(file)

            created_count = 0
            updated_count = 0

            for item in rod_data:
                name = item.get('name', '')
                if not name:
                    continue

                rod_type = item.get('rod_type', '')
                fields = {
                ...
                lookup = {'name': name, 'rod_type': rod_type}

                if options['update']:
                    _, created = Rod.objects.update_or_create(
                    ...
                    _, created = Rod.objects.get_or_create(
                        **lookup, defaults=fields,
                    )
                    if created:
                        created_count += 1

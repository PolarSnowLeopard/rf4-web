...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['clear']:
            deleted_count = Groundbait.objects.all().delete()[0]
            self.stdout.write(self.style.SUCCESS(f'已删除 {deleted_count} 条现有诱饵数据'))
        ...
        try:
            ...
            for item in data:
                ...
                groundbait_type = item.get('groundbait_type', '')
                fields = {
                ...
                lookup = {'name': name, 'groundbait_type': groundbait_type}

                if options['update']:
                    _, created = Groundbait.objects.update_or_create(
                    ...
                    _, created = Groundbait.objects.get_or_create(
                        **lookup, defaults=fields,
                    )
                    if created:
                        created_count += 1

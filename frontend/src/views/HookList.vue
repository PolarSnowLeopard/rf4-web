<template>
  <div class="container">
    <div class="header">
      <h1 class="title">钓钩图鉴</h1>
      <div class="search-bar">
        <a-input-search placeholder="搜索钓钩名称" v-model:value="searchText" style="width: 250px" @search="handleSearch" allow-clear />
        <a-select placeholder="按类型筛选" style="width: 180px; margin-left: 10px;" v-model:value="selectedType" @change="handleTypeFilter" allow-clear>
          <a-select-option v-for="t in hookTypes" :key="t" :value="t">{{ t }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card v-for="hook in list" :key="hook.id" class="item-card" :hoverable="true" @click="goToDetail(hook.id)">
          <template #cover>
            <div class="img-container"><img :src="hook.img" :alt="hook.name" class="item-image" /></div>
          </template>
          <template #title><div class="item-title">{{ hook.name }}</div></template>
          <template #extra><a-tag :color="getTypeColor(hook.hook_type)">{{ hook.hook_type }}</a-tag></template>
          <div class="item-info">
            <div class="info-row" v-if="hook.size"><span class="info-label">大小：</span><span class="info-value">{{ hook.size }}</span></div>
          </div>
        </a-card>
      </div>
      <div v-if="list.length === 0 && !loading" class="empty-result"><a-empty description="未找到符合条件的钓钩" /></div>
      <div class="pagination-container">
        <a-pagination v-model:current="page" :total="total" :pageSize="pageSize" show-quick-jumper @change="handlePageChange" />
      </div>
    </div>

    <div v-if="loading && page === 1" class="loading"><a-spin tip="加载中..." size="large"><div class="loading-content">正在获取钓钩数据...</div></a-spin></div>
  </div>
</template>
...
import { getHookList, getHookTypes } from '@/api/hook'
...
onMounted(async () => {
  ...
  try { const r = await getHookTypes(); if (r && r.data) hookTypes.value = r.data } catch (e) { /* ignore */ }
  fetchList()
})
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.hook_type = selectedType.value
    const res = await getHookList(params)
    if (res && res.data && res.data.results) { list.value = res.data.results; total.value = res.data.count || 0 }
    else { list.value = []; total.value = 0 }
  } catch (error) { console.error('获取钓钩数据失败:', error); list.value = [] }
  finally { loading.value = false }
}

const itemCount = computed(() => searchText.value || selectedType.value ? `筛选结果: ${total.value} 种钓钩` : `共 ${total.value} 种钓钩`)
...
const getTypeColor = (type) => {
  const colors = { '传统的': 'blue', '鲤鱼式': 'green', '曲柄钩': 'orange', '三本钩': 'red', '铅头钩': 'purple', '配重钩': 'cyan', '海钩': 'geekblue', '双钩': 'magenta', '饵鱼钩': 'gold', '死鱼铅头钩': 'volcano' }
  return colors[type] || 'default'
}

const goToDetail = (id) => { updateUrlParams(); router.push(`/manue/hook/${id}`) }
</script>

<template>
  <div class="detail-container" v-if="detail">
    <div class="detail-header">
      ...
      <a-tag :color="getTypeColor(detail.food_type)" class="type-tag">{{ detail.food_type }}</a-tag>
    </div>
    <div class="detail-content">
      ...
      <div class="detail-right">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="类型" v-if="detail.food_type">{{ detail.food_type }}</a-descriptions-item>
          <a-descriptions-item label="效果" v-if="detail.effect">{{ detail.effect }}</a-descriptions-item>
  ...
  <div v-else class="not-found"><a-result status="404" title="未找到该食品" sub-title="请检查链接是否正确"><template #extra><a-button type="primary" @click="goBack">返回列表</a-button></template></a-result></div>
</template>
...
onMounted(async () => {
  try { const res = await getFoodDetail(route.params.id); if (res && res.data) detail.value = res.data }
  catch (error) { console.error('获取食品详情失败:', error) }
  finally { loading.value = false }
})

const goBack = () => { router.push('/manue/food') }

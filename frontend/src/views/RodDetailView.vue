<template>
  <div class="detail-container" v-if="detail">
    <div class="detail-header">
      ...
      <a-tag :color="getTypeColor(detail.rod_type)" class="type-tag">{{ detail.rod_type }}</a-tag>
    </div>
    <div class="detail-content">
      ...
      <div class="detail-right">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="类型" v-if="detail.rod_type">{{ detail.rod_type }}</a-descriptions-item>
          <a-descriptions-item label="灵敏度" v-if="detail.sensitivity">{{ detail.sensitivity }}</a-descriptions-item>
          <a-descriptions-item label="结构" v-if="detail.structure">{{ detail.structure }}</a-descriptions-item>
          <a-descriptions-item label="硬度" v-if="detail.hardness">{{ detail.hardness }}</a-descriptions-item>
          <a-descriptions-item label="能力" v-if="detail.capacity">{{ detail.capacity }}</a-descriptions-item>
          <a-descriptions-item label="长度" v-if="detail.length">{{ detail.length }} 米</a-descriptions-item>
          <a-descriptions-item label="强度" v-if="detail.strength">{{ detail.strength }} 公斤</a-descriptions-item>
          <a-descriptions-item label="等级要求" v-if="detail.level_req">{{ detail.level_req }}</a-descriptions-item>
          <a-descriptions-item label="适配重" v-if="detail.cast_weight">{{ detail.cast_weight }}</a-descriptions-item>
  ...
  <div v-else class="not-found"><a-result status="404" title="未找到该渔竿" sub-title="请检查链接是否正确"><template #extra><a-button type="primary" @click="goBack">返回列表</a-button></template></a-result></div>
</template>
...
import { getRodDetail } from '@/api/rod'
...
onMounted(async () => {
  try { const res = await getRodDetail(route.params.id); if (res && res.data) detail.value = res.data }
  catch (error) { console.error('获取渔竿详情失败:', error) }
  finally { loading.value = false }
})

const goBack = () => { router.push('/manue/rod') }

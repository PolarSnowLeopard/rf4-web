<template>
  <div class="container">
    <div class="header">
      <h1 class="title">食品图鉴</h1>
      <div class="search-bar">
        <a-input-search placeholder="搜索食品名称" v-model:value="searchText" style="width: 250px" @search="handleSearch" allow-clear />
        <a-select placeholder="按类型筛选" style="width: 200px; margin-left: 10px;" v-model:value="selectedType" @change="handleTypeFilter" allow-clear>
          <a-select-option v-for="t in foodTypes" :key="t" :value="t">{{ t }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card v-for="item in list" :key="item.id" class="item-card" :hoverable="true" @click="goToDetail(item.id)">
          ...
          <template #extra><a-tag :color="getTypeColor(item.food_type)">{{ item.food_type }}</a-tag></template>
          <div class="item-info">
            <div class="info-row" v-if="item.effect"><span class="info-label">效果：</span><span class="info-value">{{ item.effect }}</span></div>
          </div>
        </a-card>
      </div>
      <div v-if="list.length === 0 && !loading" class="empty-result"><a-empty description="未找到符合条件的食品" /></div>
      <div class="pagination-container">
        <a-pagination v-model:current="page" :total="total" :pageSize="pageSize" show-quick-jumper @change="handlePageChange" />
      </div>
    </div>

    <div v-if="loading && page === 1" class="loading"><a-spin tip="加载中..." size="large"><div class="loading-content">正在获取食品数据...</div></a-spin></div>
  </div>
</template>
...
import { getFoodList, getFoodTypes } from '@/api/food'
...
onMounted(async () => {
  ...
  try { const r = await getFoodTypes(); if (r && r.data) foodTypes.value = r.data } catch (e) { /* ignore */ }
  fetchList()
})
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.food_type = selectedType.value
  ...
  } catch (error) { console.error('获取食品数据失败:', error); list.value = [] }
  finally { loading.value = false }
}

const itemCount = computed(() => searchText.value || selectedType.value ? `筛选结果: ${total.value} 种食品` : `共 ${total.value} 种食品`)
...
const getTypeColor = (type) => {
  const colors = { '酒（商）': 'red', '饮料（商）': 'blue', '食物（商）': 'green', '药物': 'purple', '新年博览会': 'magenta', '其他': 'default' }
  return colors[type] || 'orange'
}

const goToDetail = (id) => { updateUrlParams(); router.push(`/manue/food/${id}`) }
</script>
...
.info-value { font-weight: 500; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px; }

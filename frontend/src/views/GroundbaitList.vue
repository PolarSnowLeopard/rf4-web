<template>
  <div class="container">
    <div class="header">
      <h1 class="title">诱饵图鉴</h1>
      <div class="search-bar">
        <a-input-search placeholder="搜索诱饵名称" v-model:value="searchText" style="width: 250px" @search="handleSearch" allow-clear />
        <a-select placeholder="按类型筛选" style="width: 180px; margin-left: 10px;" v-model:value="selectedType" @change="handleTypeFilter" allow-clear>
          <a-select-option v-for="t in gbTypes" :key="t" :value="t">{{ t }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card v-for="item in list" :key="item.id" class="item-card" :hoverable="true" @click="goToDetail(item.id)">
          ...
          <template #extra><a-tag :color="getTypeColor(item.groundbait_type)">{{ item.groundbait_type }}</a-tag></template>
          <div class="item-info">
            <div class="info-row" v-if="item.weight"><span class="info-label">重量：</span><span class="info-value">{{ item.weight }}</span></div>
          </div>
        </a-card>
      </div>
      <div v-if="list.length === 0 && !loading" class="empty-result"><a-empty description="未找到符合条件的诱饵" /></div>
      <div class="pagination-container">
        <a-pagination v-model:current="page" :total="total" :pageSize="pageSize" show-quick-jumper @change="handlePageChange" />
      </div>
    </div>

    <div v-if="loading && page === 1" class="loading"><a-spin tip="加载中..." size="large"><div class="loading-content">正在获取诱饵数据...</div></a-spin></div>
  </div>
</template>
...
import { getGroundbaitList, getGroundbaitTypes } from '@/api/groundbait'
...
onMounted(async () => {
  ...
  try { const r = await getGroundbaitTypes(); if (r && r.data) gbTypes.value = r.data } catch (e) { /* ignore */ }
  fetchList()
})
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.groundbait_type = selectedType.value
    const res = await getGroundbaitList(params)
    if (res && res.data && res.data.results) { list.value = res.data.results; total.value = res.data.count || 0 }
    else { list.value = []; total.value = 0 }
  } catch (error) { console.error('获取诱饵数据失败:', error); list.value = [] }
  finally { loading.value = false }
}

const itemCount = computed(() => searchText.value || selectedType.value ? `筛选结果: ${total.value} 种诱饵` : `共 ${total.value} 种诱饵`)
...
const getTypeColor = (type) => {
  const colors = { '基础料': 'blue', '引诱': 'green', '添加剂': 'orange', '聚乙烯醇材料': 'purple' }
  return colors[type] || 'default'
}

const goToDetail = (id) => { updateUrlParams(); router.push(`/manue/groundbait/${id}`) }
</script>

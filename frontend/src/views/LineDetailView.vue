<template>
  <div class="detail-container" v-if="detail">
    <div class="detail-header">
      ...
      <a-tag :color="getTypeColor(detail.line_type)" class="type-tag">{{ detail.line_type }}</a-tag>
    </div>
    <div class="detail-content">
      ...
      <div class="detail-right">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="类型" v-if="detail.line_type">{{ detail.line_type }}</a-descriptions-item>
          <a-descriptions-item label="形式" v-if="detail.form">{{ detail.form }}</a-descriptions-item>
          <a-descriptions-item label="颜色" v-if="detail.color">{{ detail.color }}</a-descriptions-item>
  ...
  <div v-else class="not-found"><a-result status="404" title="未找到该鱼线" sub-title="请检查链接是否正确"><template #extra><a-button type="primary" @click="goBack">返回列表</a-button></template></a-result></div>
</template>
...
onMounted(async () => {
  try { const res = await getLineDetail(route.params.id); if (res && res.data) detail.value = res.data }
  catch (error) { console.error('获取鱼线详情失败:', error) }
  finally { loading.value = false }
})

const goBack = () => { router.push('/manue/line') }

<template>
  <div class="container">
    <div class="header">
      <h1 class="title">鱼线图鉴</h1>
      <div class="search-bar">
        <a-input-search placeholder="搜索鱼线名称" v-model:value="searchText" style="width: 250px" @search="handleSearch" allow-clear />
        <a-select placeholder="按类型筛选" style="width: 180px; margin-left: 10px;" v-model:value="selectedType" @change="handleTypeFilter" allow-clear>
          <a-select-option v-for="t in lineTypes" :key="t" :value="t">{{ t }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card v-for="line in list" :key="line.id" class="item-card" :hoverable="true" @click="goToDetail(line.id)">
          <template #cover>
            <div class="img-container"><img :src="line.img" :alt="line.name" class="item-image" /></div>
          </template>
          <template #title><div class="item-title">{{ line.name }}</div></template>
          <template #extra><a-tag :color="getTypeColor(line.line_type)">{{ line.line_type }}</a-tag></template>
          <div class="item-info">
            <div class="info-row" v-if="line.length"><span class="info-label">长度：</span><span class="info-value">{{ line.length }}</span></div>
            <div class="info-row" v-if="line.color"><span class="info-label">颜色：</span><span class="info-value">{{ line.color }}</span></div>
          </div>
        </a-card>
      </div>
      <div v-if="list.length === 0 && !loading" class="empty-result"><a-empty description="未找到符合条件的鱼线" /></div>
      <div class="pagination-container">
        <a-pagination v-model:current="page" :total="total" :pageSize="pageSize" show-quick-jumper @change="handlePageChange" />
      </div>
    </div>

    <div v-if="loading && page === 1" class="loading"><a-spin tip="加载中..." size="large"><div class="loading-content">正在获取鱼线数据...</div></a-spin></div>
  </div>
</template>
...
import { getLineList, getLineTypes } from '@/api/line'
...
onMounted(async () => {
  ...
  try { const r = await getLineTypes(); if (r && r.data) lineTypes.value = r.data } catch (e) { /* ignore */ }
  fetchList()
})
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.line_type = selectedType.value
  ...
  } catch (error) { console.error('获取鱼线数据失败:', error); list.value = [] }
  finally { loading.value = false }
}

const itemCount = computed(() => searchText.value || selectedType.value ? `筛选结果: ${total.value} 种鱼线` : `共 ${total.value} 种鱼线`)
...
const getTypeColor = (type) => {
  const colors = { '编织线绳': 'blue', '尼龙线': 'green', '氟碳线': 'orange', '新年系列鱼线': 'red' }
  return colors[type] || 'default'
}

const goToDetail = (id) => { updateUrlParams(); router.push(`/manue/line/${id}`) }
</script>

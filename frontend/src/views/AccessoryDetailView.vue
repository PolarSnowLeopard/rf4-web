<template>
  <div class="detail-container" v-if="detail">
    <div class="detail-header">
      ...
      <a-tag :color="getTypeColor(detail.accessory_type)" class="type-tag">{{ detail.accessory_type }}</a-tag>
    </div>
    <div class="detail-content">
      ...
      <div class="detail-right">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="类型" v-if="detail.accessory_type">{{ detail.accessory_type }}</a-descriptions-item>
        </a-descriptions>
  ...
  <div v-else class="not-found"><a-result status="404" title="未找到该辅助用品" sub-title="请检查链接是否正确"><template #extra><a-button type="primary" @click="goBack">返回列表</a-button></template></a-result></div>
</template>
...
onMounted(async () => {
  try { const res = await getAccessoryDetail(route.params.id); if (res && res.data) detail.value = res.data }
  catch (error) { console.error('获取辅助用品详情失败:', error) }
  finally { loading.value = false }
})

const goBack = () => { router.push('/manue/accessory') }

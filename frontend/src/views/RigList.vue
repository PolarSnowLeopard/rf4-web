<template>
  <div class="container">
    <div class="header">
      <h1 class="title">钓组图鉴</h1>
      <div class="search-bar">
        <a-input-search placeholder="搜索钓组名称" v-model:value="searchText" style="width: 250px" @search="handleSearch" allow-clear />
        <a-select placeholder="按类型筛选" style="width: 180px; margin-left: 10px;" v-model:value="selectedType" @change="handleTypeFilter" allow-clear>
          <a-select-option v-for="t in rigTypes" :key="t" :value="t">{{ t }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card v-for="rig in list" :key="rig.id" class="item-card" :hoverable="true" @click="goToDetail(rig.id)">
          <template #cover>
            <div class="img-container"><img :src="rig.img" :alt="rig.name" class="item-image" /></div>
          </template>
          <template #title><div class="item-title">{{ rig.name }}</div></template>
          <template #extra><a-tag :color="getTypeColor(rig.rig_type)">{{ rig.rig_type }}</a-tag></template>
          <div class="item-info">
            <div class="info-row" v-if="rig.weight"><span class="info-label">重量：</span><span class="info-value">{{ rig.weight }}</span></div>
          </div>
        </a-card>
      </div>
      <div v-if="list.length === 0 && !loading" class="empty-result"><a-empty description="未找到符合条件的钓组" /></div>
      <div class="pagination-container">
        <a-pagination v-model:current="page" :total="total" :pageSize="pageSize" show-quick-jumper @change="handlePageChange" />
      </div>
    </div>

    <div v-if="loading && page === 1" class="loading"><a-spin tip="加载中..." size="large"><div class="loading-content">正在获取钓组数据...</div></a-spin></div>
  </div>
</template>
...
import { getRigList, getRigTypes } from '@/api/rig'
...
onMounted(async () => {
  ...
  try { const r = await getRigTypes(); if (r && r.data) rigTypes.value = r.data } catch (e) { /* ignore */ }
  fetchList()
})
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.rig_type = selectedType.value
    const res = await getRigList(params)
    if (res && res.data && res.data.results) { list.value = res.data.results; total.value = res.data.count || 0 }
    else { list.value = []; total.value = 0 }
  } catch (error) { console.error('获取钓组数据失败:', error); list.value = [] }
  finally { loading.value = false }
}

const itemCount = computed(() => searchText.value || selectedType.value ? `筛选结果: ${total.value} 种钓组` : `共 ${total.value} 种钓组`)
...
const goToDetail = (id) => { updateUrlParams(); router.push(`/manue/rig/${id}`) }
</script>

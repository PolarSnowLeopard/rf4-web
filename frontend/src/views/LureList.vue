  ...
  <div class="container">
    <div class="header">
      ...
      <div class="search-bar">
        ...
        <a-select
          ...
          allow-clear
        >
          <a-select-option v-for="t in lureTypes" :key="t" :value="t">
            {{ t }}
          </a-select-option>
        </a-select>
      </div>
    ...
    <div v-if="!loading || page > 1">
      ...
      <div class="card-container">
        <a-card
          ...
          class="item-card"
          :hoverable="true"
          @click="goToDetail(lure.id)"
        >
          <template #cover>
            <div class="img-container">
              <img :src="lure.img" :alt="lure.name" class="item-image" />
            </div>
          </template>
          <template #title>
            <div class="item-title">{{ lure.name }}</div>
          </template>
          <template #extra>
            <a-tag :color="getTypeColor(lure.lure_type)">{{ lure.lure_type }}</a-tag>
          </template>
          <div class="item-info">
            <div class="info-row" v-if="lure.weight">
              <span class="info-label">质量：</span>
              <span class="info-value">{{ lure.weight }}</span>
            </div>
            <div class="info-row" v-if="lure.length">
              <span class="info-label">长度：</span>
              <span class="info-value">{{ lure.length }}</span>
            </div>
          </div>
        </a-card>
      </div>
      ...
      <div v-if="list.length === 0 && !loading" class="empty-result">
        <a-empty description="未找到符合条件的拟饵" />
      </div>
    ...
    <div v-if="loading && page === 1" class="loading">
      <a-spin tip="加载中..." size="large">
        <div class="loading-content">正在获取拟饵数据...</div>
      </a-spin>
    </div>
  </div>
</template>
...
import { getLureList, getLureTypes } from '@/api/lure'
...
onMounted(async () => {
  ...
  try {
    const typesRes = await getLureTypes()
    if (typesRes && typesRes.data) lureTypes.value = typesRes.data
  } catch (e) { /* ignore */ }
...
const fetchList = async () => {
  try {
    ...
    if (selectedType.value) params.lure_type = selectedType.value

    const res = await getLureList(params)
...
const itemCount = computed(() => {
  if (searchText.value || selectedType.value) return `筛选结果: ${total.value} 种拟饵`
  return `共 ${total.value} 种拟饵`
})

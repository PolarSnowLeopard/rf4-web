<template>
  <div class="detail-container" v-if="detail">
    <div class="detail-header">
      ...
      <a-tag :color="getTypeColor(detail.hook_type)" class="type-tag">{{ detail.hook_type }}</a-tag>
    </div>
    <div class="detail-content">
      ...
      <div class="detail-right">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="类型" v-if="detail.hook_type">{{ detail.hook_type }}</a-descriptions-item>
  ...
  <div v-else class="not-found"><a-result status="404" title="未找到该钓钩" sub-title="请检查链接是否正确"><template #extra><a-button type="primary" @click="goBack">返回列表</a-button></template></a-result></div>
</template>
...
import { getHookDetail } from '@/api/hook'
...
onMounted(async () => {
  try { const res = await getHookDetail(route.params.id); if (res && res.data) detail.value = res.data }
  catch (error) { console.error('获取钓钩详情失败:', error) }
  finally { loading.value = false }
})

const goBack = () => { router.push('/manue/hook') }
